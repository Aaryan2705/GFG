...
class Solution {
  public:
    Node* cloneLinkedList(Node* head) {
        // Step 1: Create clones and populate the hashmap
        Node* temp = head;
        unordered_map<Node*, Node*> nodeMap; // Map original -> cloned nodes
        
        while (temp != nullptr) {
            nodeMap[temp] = new Node(temp->data); // Create a clone for each node
            temp = temp->next;
        }
        
        // Step 2: Set next and random pointers for the cloned nodes
        temp = head;
        while (temp != nullptr) {
            nodeMap[temp]->next = nodeMap[temp->next];   // Set the next pointer
            nodeMap[temp]->random = nodeMap[temp->random]; // Set the random pointer
            temp = temp->next;
        }
